...
//
//   - `iedora app apply` (in-process) — configurators.go wires Run
//     directly with no args (defaults: --mode live, no --grants-only).
//   - `bin/zitadel-apply` (separate process) — thin wrapper at
//     infra/app-state/cmd/zitadel-apply/main.go installs signal handling
//     and parses os.Args, then calls Run.
...
// Run is the configurator entry point — same surface for both invocation
// paths. The caller (orchestrator or bin shim) owns context cancellation,
// argv parsing scope, and exit code translation; Run just returns an
// error and lets the caller decide. argv is the raw flag slice (excludes
// argv[0]); pass nil from the orchestrator for defaults.
