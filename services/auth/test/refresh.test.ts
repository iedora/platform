...
import { json, PASSWORD, refreshCookie, registerUser, useHarness, withCookie } from "./harness";
...
test("refresh rotates the token; the old token is reuse-detected and burns the family", async () => {
  ...
  const r1 = await h.app.request("/auth/refresh", withCookie(c1));
  ...
  expect((await h.app.request("/auth/refresh", withCookie(c1))).status).toBe(401);
  // …and the family is burned, so the successor is dead too.
  expect((await h.app.request("/auth/refresh", withCookie(c2))).status).toBe(401);
});
