...
// The audit service's own wire format (vendored — no shared contracts package).
// Responses validate against these; any reader consumes the inferred types.
...
export const auditRecord = z.object({
  ...
  // Request context. `ip` is the raw client IP; `userAgent` is the client
  // string. Both optional — historical rows and system/service events carry
  // neither.
...
export const auditFilter = z.object({
  ...
  target: z.string().optional(), // exact target_id
